...
def get_apr_from_cakedefi(coin_pair: tuple[str, str]) -> int:
    ...
    for i in coin_pair_blocks:
        ...
        if re.match(f"(?i){coin_pair[0]}\n?-\n?{coin_pair[1]}", i.get_text().strip()) or \
           re.match(f"(?i){coin_pair[1]}\n?-\n?{coin_pair[0]}", i.get_text().strip()): # e.g. "BCH-DFI" or "DFI-BCH"
            ...
            if re.match(r"\bAPR[0-9]?", apr_block.get_text().strip()): # (?i): case insensitive; \b: match beginning of a word
